...
def Load_amorphous_data():
    return pd.read_excel('비정형.xlsx', engine='openpyxl') # Load 비정형 데이터

def Load_material_data():
    return pd.read_excel('소모품.xlsx', engine='openpyxl') # Load 재료 데이터

def show_amorphous_search(): # 비정형 검색함수
    clear_frame()
    ...
    label1 = tk.Label(entry_frame, text="KeyWord [1]")
    ...
    label2 = tk.Label(entry_frame, text="KeyWord [2]")
    ...
    label3 = tk.Label(entry_frame, text="KeyWord [3]")
    ...
    search_button = tk.Button(entry_frame, text="검색", command=search) #Serach Btn
    search_button.grid(row=0, column=6, padx=5)

    global listbox  #Serach Result Btn
    listbox = tk.Listbox(frame, width=150, height=15)
    listbox.pack(pady=5)

    back_button = tk.Button(frame, text="뒤로가기", command=show_main_menu) #뒤로가기 버튼
    back_button.pack(pady=5)

def show_material_search(): #재료검색 기능
    clear_frame()
...
def material_search(term):
    ...
    if term.strip() == "":
        messagebox.showwarning("Warning", "Please input KeyWord")
        return
...
def show_main_menu():
    ...
    unstructured_button = tk.Button(frame, text="비정형 검색", image=main_image, compound=tk.TOP, command=show_amorphous_search)
...
# def show_main_menu():
#     clear_frame()
#     unstructured_button = tk.Button(frame, text="비정형 검색기", command=show_amorphous_search)
